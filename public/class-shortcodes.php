...
class NVB_Shortcodes {
	...
	public static function homepage( $atts = array(), $content = null ) {
		ob_start();
	...
	public static function country_detail( $atts = array() ) {
		...
		// اگر shortcode میں id خالی ہو تو URL سے ?country=slug پکڑیں
		if ( empty( $atts['id'] ) && isset( $_GET['country'] ) ) {
			$atts['id'] = sanitize_text_field( wp_unslash( $_GET['country'] ) );
		}

		// اگر پھر بھی id خالی ہو تو error
		if ( empty( $atts['id'] ) ) {
	...
	public static function export_checklist_csv() {
		...
		$rows   = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT title, is_required, note 
				 FROM {$prefix}nvb_documents 
				 WHERE country_id = %d AND is_deleted = 0",
				$country_id
			),
			ARRAY_A
		);
