...
<div class="wrap nvb-admin-wrap">
    ...
    <form method="get" action="">
        ...
        <p class="search-box">
            <label class="screen-reader-text" for="nvb-search">Search Steps</label>
            <input type="search" id="nvb-search" name="s" value="<?php echo isset($_GET['s']) ? esc_attr($_GET['s']) : ''; ?>" />
    ...
    <table class="nvb-list-table">
        ...
        <?php if ($steps) : foreach ($steps as $st) : ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo admin_url('admin.php?page=nvb_application_steps&action=edit&id=' . $st->id); ?>">Edit</a>
                     |
                    <a class="nvb-confirm-delete"
                       href="<?php echo wp_nonce_url(
                           admin_url('admin-post.php?action=nvb_delete_application_step&id=' . $st->id),
        ...
        <?php endforeach; else : ?>
            <tr><td colspan="5">No Application Steps found.</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
